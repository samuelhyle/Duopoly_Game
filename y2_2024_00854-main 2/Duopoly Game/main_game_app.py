...
class ProfitFunction(QWidget):
    def __init__(self):
        ...
        self.chart_label = QLabel("PROFIT GRAPH")
        self.label_v = QHBoxLayout()
        self.label_v.addWidget(self.chart_label)
        self.chart_view_vv = QVBoxLayout()
        self.chart_view_vv.addLayout(self.label_v)
...
class Widget(QWidget):
    ...
    def __init__(self):
        ...
        self.add_q.setEnabled(False)

        self.input_f = QHBoxLayout()
        self.input_f.addLayout(self.q_input_v)
        self.input_f.addLayout(self.add_q_v)

        self.q_input_label = QLabel(f"ENTER THE PRODUCTION QUANTITY FOR THE NEXT ROUND")
        ...
        self.end_button = QPushButton("END THE GAME")
        self.end_f = QHBoxLayout()
        self.end_f.addWidget(self.end_button)
        ...
        self.end_button.clicked.connect(self.game_is_over)
    ...
    def play(self):
        ...
        if self.round < 12:
            ...
            self.text_box.insertPlainText("The industry has seen a shift in the demand, "
                                          "and each firm has also experienced changes in their costs. "
                                          "The updated states can be found below.")
            ...
            self.text_box.insertPlainText("The industry has seen a shift in the demand, "
                                          "and each firm has also experienced changes in their costs. "
                                          "The updated states can be found below. "
                                          "This data applies for the coming rounds.\n")

        elif 12 < self.round <= 24:
            ...
            if self.firm_one_profit > self.firm_two_profit:

                self.text_box.setText(f"The game has ended, the winner of the game is firm 1 with a whopping profit of {self.firm_one_profit},"
                                      f" and firm 2 losing the game with profit of {self.firm_two_profit}. You can quit the game by pressing END GAME")
                self.end_button.setEnabled(True)
                self.add_q.setEnabled(False)

            elif self.firm_two_profit > self.firm_one_profit:

                self.text_box.setText(
                    f"The game has ended, the winner of the game is firm 2 with a whopping profit of {self.firm_two_profit},"
                    f" and firm 1 losing the game with profit of {self.firm_one_profit}. You can quit the game by pressing END GAME")
                self.end_button.setEnabled(True)
                self.add_q.setEnabled(False)

            elif self.firm_one_profit == self.firm_two_profit:

                self.text_box.setText(
                    f"The game has ended, the game ended with a tie, with firm 1 producing {self.firm_one_profit}, and firm 2 producing {self.firm_two_profit}. You can quit the game by pressing END GAME")
                self.end_button.setEnabled(True)
                self.add_q.setEnabled(False)
    ...
    def new_set(self):
        ...
        total = int(self.total_in.value())
        b = int(self.b_in.value())
        c1 = int(self.c1_in.value())
        c2 = int(self.c2_in.value())
        ...
        t_new = randint(t_1, t_2)
        c1_new = randint(c1_1, c1_2)
        c2_new = randint(c2_1, c2_2)

        data = [t_new, b, c1_new, c2_new]
