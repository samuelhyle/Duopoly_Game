...
def profit_function(x1,x2,c1,b):
    return demand_function(x1,x2,b)*x1-cost_function(x1,c1)


def reaction_function(x2,c1,b):
    x1 = optimize.brute(lambda x: -profit_function(x,x2,c1,b), ((0,1,),))
    return x1[0]


def v_reaction_function(x,param):
    return array(x)-array([reaction_function(x[1],param[1],param[0]),reaction_function(x[0],param[2],param[0])])
...
def init_matrix_tables(values, stats):
    ...
    firm_one_profit = demand_nash * firm_one_full_output - cost_function(firm_one_full_output, c1)
    firm_two_profit = demand_nash * firm_two_full_output - cost_function(firm_two_full_output, c2)
    ...
    cartel_profit_one = cartel_demand * cartel_output_firm - cost_function(cartel_output_firm, c1)
    cartel_profit_two = cartel_demand * cartel_output_firm - cost_function(cartel_output_firm, c2)
    ...
    cartel_break_profit_one = cartel_break_demand * cartel_break_output - cost_function(cartel_break_output, c1)
    cartel_break_profit_two = cartel_break_demand * cartel_break_output - cost_function(cartel_break_output, c2)

    cartel_stay_profit_one = cartel_break_demand * cartel_output_firm - cost_function(cartel_output_firm, c1)
    cartel_stay_profit_two = cartel_break_demand * cartel_output_firm - cost_function(cartel_output_firm, c2)
...
def init_nash_states(initial_values):
    ...
    stats = optimize.fsolve(v_reaction_function, x0, args=params)
